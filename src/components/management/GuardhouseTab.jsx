...
export default function GuardhouseTab({ 
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-200">
      ...
      <div className="rounded-2xl bg-white dark:bg-zinc-950/80 border border-slate-200 dark:border-white/[0.08] shadow-rim p-6 space-y-5 backdrop-blur-md">
        <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400 font-semibold text-sm border-b border-slate-200 dark:border-white/[0.08] pb-3">
        ...
        <form onSubmit={handleVerifyPass} className="space-y-4">
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                type="submit"
                className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-black font-semibold text-xs rounded-xl shadow-rim transition-colors whitespace-nowrap cursor-pointer min-h-[44px]"
              >
                Sahkan Kod
              </button>
            </div>
      ...
      <div className="rounded-2xl bg-white dark:bg-zinc-950/80 border border-slate-200 dark:border-white/[0.08] shadow-rim p-6 space-y-5 backdrop-blur-md">
        ...
        <form onSubmit={handleLogParcelSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-2.5 px-4 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-semibold text-xs shadow-rim transition-colors flex items-center justify-center gap-2 cursor-pointer min-h-[44px]"
          >
        ...
        {/* Recent Parcels List */}
        <div className="pt-2">
          <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-zinc-400 mb-2">Bungkusan Menunggu Kutipan</h4>
          <div className="space-y-2">
            {parcels.filter(p => p.status === 'AWAITING_PICKUP').map(p => (
              <div key={p.id} className="p-3 rounded-xl bg-slate-50 dark:bg-zinc-900/50 border border-slate-200 dark:border-white/[0.06] hover:border-slate-300 dark:hover:border-white/[0.12] flex items-center justify-between text-xs shadow-rim transition-all">
                <div>
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-emerald-500/10 dark:bg-zinc-800 dark:hover:bg-emerald-500/20 text-slate-700 hover:text-emerald-700 dark:text-zinc-300 dark:hover:text-emerald-300 text-[11px] font-medium border border-slate-200 dark:border-white/[0.08] transition-colors cursor-pointer min-h-[44px]"
                >
                  Tuntut (Collected)
                </button>
              </div>
