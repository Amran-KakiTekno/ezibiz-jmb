  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-200">
      ...
      <div className="rounded-2xl bg-slate-900 border border-slate-800 p-6 space-y-4">
        <div>
          <h3 className="font-bold text-white text-base">Hebahan Notis Rasmi JMB</h3>
          <p className="text-xs text-slate-400">Siaran ini akan disematkan di bahagian atas suapan semua residen.</p>
        </div>

        <form onSubmit={handlePublish} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Tajuk Pengumuman</label>
            <input
              type="text"
              value={noticeTitle}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Kandungan Notis</label>
            <textarea
              rows={4}
              value={noticeContent}
      ...
      <div className="lg:col-span-2 space-y-4">
        <div>
          <h3 className="font-bold text-white text-base">Resolusi AGM & Undian Statutori</h3>
          <p className="text-xs text-slate-400">Sistem e-undian telus mengikut syarat kuorum Akta Pengurusan Strata 2013.</p>
        </div>
        ...
        <div className="space-y-4">
          {resolutions.map(res => {
            ...
            return (
              <div key={res.id} className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-4">
                ...
                <div className="space-y-1.5 bg-slate-950 p-3.5 rounded-xl border border-slate-800">
                  <div className="flex justify-between text-xs">
                    <span className="text-slate-400">Persetujuan Kuorum ({approvalPercent}%)</span>
                ...
                {res.status === 'VOTING_ACTIVE' && (
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      ...
                      className="flex-1 py-2 px-3 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-300 font-semibold text-xs transition-colors"
                    >
                      ✓ Sokong (Undi Ya)
                    </button>
                    <button
                      ...
                      className="flex-1 py-2 px-3 rounded-lg bg-rose-600/20 hover:bg-rose-600/30 border border-rose-500/30 text-rose-300 font-semibold text-xs transition-colors"
                    >
                      ✕ Bantah (Undi Tidak)
                    </button>
                  </div>
