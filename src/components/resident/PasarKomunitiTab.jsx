    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap cursor-pointer ${
                communityFilter === cat
                  ...
                  : 'bg-zinc-900/80 text-zinc-400 hover:text-white border border-white/[0.08] shadow-rim'
              }`}
            >
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-semibold text-xs shadow-rim transition-colors whitespace-nowrap cursor-pointer"
        >
        ...
        {filteredPosts.map(post => (
          <div 
            ...
            className={`rounded-2xl p-5 border transition-all flex flex-col justify-between ${
              post.pinned 
                ? 'bg-gradient-to-r from-zinc-950 to-amber-950/20 border-amber-500/30 shadow-rim'
                : 'bg-zinc-950/80 border-white/[0.08] shadow-rim hover:border-white/[0.15]'
            }`}
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-full bg-zinc-900 border border-white/[0.08] shadow-rim flex items-center justify-center font-bold text-xs text-amber-400">
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-zinc-500">{post.timestamp}</span>
                  </div>
                </div>

                {post.price && (
                  <span className="font-mono tabular-nums font-bold text-amber-400 text-xs px-2.5 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 shadow-rim">
                    {post.price}
                  </span>
                )}
              </div>

              <h4 className="font-semibold text-zinc-100 text-sm mb-2">{post.title}</h4>
              <p className="text-xs text-zinc-400 leading-relaxed">{post.content}</p>
            </div>

            <div className="mt-4 pt-3 border-t border-white/[0.08] flex items-center justify-between text-xs text-zinc-400">
              <div className="flex items-center gap-4 text-[11px]">
                <span>❤️ {post.likes}</span>
                {post.readReceiptsCount && (
                  <span className="text-emerald-400">✓ {post.readReceiptsCount} Dibaca</span>
                )}
              </div>
              ...
              {post.whatsapp && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 font-semibold text-xs transition-colors"
                >
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="relative w-full max-w-lg bg-zinc-950 border border-white/[0.1] shadow-card-elevated rounded-2xl overflow-hidden p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-white/[0.08] pb-3">
              <h3 className="font-semibold text-white text-base">Kongsi ke Komuniti Jiran</h3>
              <button
            ...
            <form onSubmit={handleCreatePost} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-zinc-300 mb-1">Saluran</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-medium text-zinc-300 mb-1">Harga / Kadar (Pilihan)</label>
                  <input
                    type="text"
                    value={newPostPrice}
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-300 mb-1">Tajuk Siaran</label>
                <input
                  type="text"
                  value={newPostTitle}
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-300 mb-1">Kandungan Terperinci</label>
                <textarea
                  rows={3}
                  value={newPostContent}
                  onChange={(e) => setNewPostContent(e.target.value)}
                  placeholder="Kongsikan butiran lanjut, masa penghantaran atau maklumat lain..."
                  className="w-full px-3 py-2 bg-zinc-900 border border-white/[0.08] rounded-xl text-xs text-zinc-200 focus:outline-none focus:border-amber-500/50 resize-none"
                ></textarea>
              </div>
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setShowCreateModal(false)}
                  className="px-4 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 text-zinc-300 text-xs font-medium border border-white/[0.08] cursor-pointer"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-black text-xs font-semibold shadow-rim cursor-pointer"
                >
                  Terbitkan Siaran
                </button>
              </div>
