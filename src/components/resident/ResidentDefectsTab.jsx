  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-200">
      <div className="rounded-2xl bg-slate-900 border border-slate-800 p-6 space-y-4">
        <div>
          <h3 className="font-bold text-white text-base">Adu Kerosakan Baharu</h3>
          <p className="text-xs text-slate-400">Pengurusan JMB komited dengan SLA maklum balas 24-48 jam.</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Kategori Masalah</label>
            <select
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Lokasi Kerosakan</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Ringkasan Aduan</label>
            <input
              type="text"
              value={defectTitle}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1">Perihal Terperinci</label>
            <textarea
              rows={3}
              value={defectDesc}
          ...
          <div className="p-3 rounded-lg border border-dashed border-slate-800 bg-slate-950/40 flex items-center justify-center gap-2 text-xs text-slate-400 cursor-pointer hover:border-amber-500/50 transition-colors">
      ...
      <div className="lg:col-span-2 space-y-4">
        <h3 className="font-bold text-white text-base">Senarai Aduan Semasa</h3>
        <div className="space-y-3">
          {defects.map(d => (
            <div key={d.id} className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs font-bold text-amber-400">{d.ticketNo}</span>
                  <span className="text-xs px-2 py-0.5 rounded bg-slate-800 text-slate-300 font-medium">
                    {d.category}
                  </span>
                  {d.isForm28Eligible && (
                    <span className="text-[10px] px-2 py-0.5 rounded bg-rose-500/15 border border-rose-500/30 text-rose-300 font-bold">
                      Statutori Borang 28
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2 text-xs">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 font-semibold font-mono text-[11px]">
                    {d.slaHours} Jam
                  </span>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-slate-800/80 flex flex-wrap items-center justify-between text-xs text-slate-400 gap-2">
                <div>
                  ...
                  <span className="text-slate-200 font-medium">{d.assignedContractor}</span>
                </div>
                <div>
                  <span>Status: </span>
                  <span className={`font-semibold ${
                    d.status === 'RESOLVED' ? 'text-emerald-400' :
                    d.status === 'CONTRACTOR_ASSIGNED' ? 'text-indigo-400' :
                    'text-amber-400'
                  }`}>
